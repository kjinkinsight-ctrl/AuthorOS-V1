...
void main() {
  ...
  group('producing the convention from a selection', () {
    test('wraps a selection', () {
      ...
      expect(result.text, 'She read _The Kestrel_ twice.');
      expect(text.substring(9, 20), 'The Kestrel');
      expect(result.text.substring(result.start, result.end), '_The Kestrel_',
          reason: 'the same prose stays selected, markers included');
    });

    test('unwraps a selection that is already wrapped', () {
      const text = 'She read _The Kestrel_ twice.';
      final result = toggleEmphasis(text, 9, 22);

      expect(result.text, 'She read The Kestrel twice.');
      expect(result.text.substring(result.start, result.end), 'The Kestrel');
    });

    test('wrapping then unwrapping gives back exactly what was there', () {
      const text = 'She read The Kestrel twice.';
      final wrapped = toggleEmphasis(text, 9, 20);
      final back = toggleEmphasis(wrapped.text, wrapped.start, wrapped.end);
      expect(back.text, text);
    });

    test('an escaped underscore is not mistaken for a wrapper', () {
      const text = '__literal__';
      expect(toggleEmphasis(text, 0, text.length).text, '___literal___');
    });

    test('an empty or invalid selection changes nothing', () {
      const text = 'unchanged';
      expect(toggleEmphasis(text, 3, 3).text, text);
      expect(toggleEmphasis(text, -1, 4).text, text);
      expect(toggleEmphasis(text, 0, 999).text, text);
    });

    test('what it produces is what the parser reads back', () {
      const text = 'She read The Kestrel twice.';
      final wrapped = toggleEmphasis(text, 9, 20).text;
      expect(italics(wrapped), ['The Kestrel'],
          reason: 'the button and the parser have to agree, or pressing it '
              'produces markers that do nothing');
    });
  });
