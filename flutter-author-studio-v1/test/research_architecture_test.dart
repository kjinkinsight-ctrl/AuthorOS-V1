...
void main() {
  ...
  group('Manuscript research side panel', () {
    test('the legacy SharedPreferences panel is left untouched and intact', () {
      // The Research Studio milestone deliberately did not migrate or delete
      // the panel, so no author loses data. The guard fails if it disappears
      // without a migration landing alongside it.
      //
      // That migration has now landed. The store moved out of `main.dart`
      // into `lib/migrations/legacy_research_store.dart` so the migration can
      // read it without depending on the app shell — same key, same JSON,
      // same behaviour, and `main.dart` still re-exports it. What this guard
      // protects is unchanged: the panel still exists, still owns the same
      // preferences key, and is still nobody's to delete yet.
      final main = File('lib/main.dart').readAsStringSync();
      final store =
          File('lib/migrations/legacy_research_store.dart').readAsStringSync();
      expect(store, contains('class ProjectResearchStore'));
      expect(store, contains('author_studio.research_panel.'));
      expect(main, contains('legacy_research_store.dart'));
      expect(main, contains('ProjectResearchStore'));

      // The panel widget itself is still wired up and still the only writer.
      expect(main, contains('_ResearchSidePanel'));
      expect(
        main,
        contains('ProjectResearchStore(projectId: widget.projectId).save'),
      );

      // And the migration reads that store rather than copying it.
      final migration =
          File('lib/migrations/research_migration.dart').readAsStringSync();
      expect(migration, contains('ProjectResearchStore'));
      expect(migration.contains('author_studio.research_panel'), isFalse);
